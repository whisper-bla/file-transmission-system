...
// 问题1：两个发送函数和两个接收函数，一个底层操作一个封包，但是数据状态是怎么更新的？我不太理解数据包的意思，感觉像是第一次发个success第二次发文件，第三次又发个success，那序列和这是第几个包这种状态什么好像没有更新？就发一个文件为什么会要数据包？
// 问题2：这个代码能不能实现发送图片或者什么其他的东西，难不成只能传送文件及其内容里的文字信息吗？
// 问题3：要实现跨平台或者不同电脑间的通信要怎么实现